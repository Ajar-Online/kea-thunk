...
function createRealThunks(
  ...
  Object.keys(output.actions).forEach(actionKey => {
    actions[actionKey] = (...actionArgs) =>
      dispatch(output.actions[actionKey](...actionArgs));
  ...
  return input.thunks(
    Object.assign({}, output, { actions, dispatch, getState, get, fetch })
  );
}
...
const injectToClass = (Klass, input, output) => {
  ...
  Klass.prototype.componentWillMount = function() {
    ...
    thunkKeys.forEach(thunkKey => {
      thunkFunctions[thunkKey] = (...args) => (dispatch, getState) => {
        if (!realThunks) {
